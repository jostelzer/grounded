...
7. **Audit and receipt** — inventory all assertions, including headings and uncited summaries. A fresh judge qualifies against unlabelled multi-domain cases, then checks source quotations, quantities, qualifications, and element coverage against the relevant methods, results, table headers and notes. New audits record the claim’s scientific meaning, inspected source ranges and interpretation separately from quote matching and evidence strength. A final whole-review check examines the reader’s takeaway and each rendered figure against checked scientific claims. Changes to source context, judgments, prose or figure files invalidate the applicable approvals. Partial source support is accepted only when other evidence covers the remaining elements. Receipts distinguish attribution, interpretation, source access and outcome certainty; historical audits are not relabelled as having passed the new checks. Deterministic checks establish consistency, not scientific truth or judge independence.
