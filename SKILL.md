...
Skip this step for inline chat. For the journal PDF format figures are mandatory: plan toward 2 for small, 3–4 for medium, and 5–6 for large, never exceeding 2/5/8; fewer is valid only when `synthesis.md` contains fewer distinct visual stories. For slides, build the deck. Read and follow `references/figure-generation-contract.md` before producing any pixels. Build visuals only from the claim and pattern entries in `synthesis.md`, never from a fresh reading of the papers. Probe the current agent's media capabilities first: when a capable built-in image generator is exposed, it is the default visual author for every conceptual, mechanistic, anatomical, study-overview, popsci, and ELI5 figure. Exact quantitative plots remain deterministic. Choose and record an explicit `generated`, `hybrid`, or `deterministic` route; hybrid means the generator creates the rich scientific composition and `scripts/compose_hybrid_figure.py` adds exact labels, arrows, legend, or values without resizing the base.

Every new figure spec uses `quality_contract_version: 1`, the review's explicit `review_style`, a numeric `target_aspect_ratio`, a `visual_anchor`, and an explicit `render_route`; include directed `relationships`, local `abbreviations`, and any shape-specific `geometry_invariants`. Build and save the mega-rich structured prompt with `scripts/build_figure_prompt.py`. Generate at least two meaningfully different candidates for a generated or hybrid route, inspect them at original and final size, make a targeted edit when the defect is local, and record the comparison in `figure.provenance.json`. Do not select a generic, cheap-looking flowchart merely because its text is easy to OCR. When dense exact copy is the only blocker, preserve the strongest generated composition and use the hybrid overlay before considering a pure deterministic fallback. A non-quantitative deterministic fallback requires two generated candidates, a targeted edit, explicit hybrid consideration, and a concrete recorded failure reason.
