...
Skip this step for inline chat. For the journal PDF format figures are mandatory: plan toward 2 for small, 3–4 for medium, and 5–6 for large, never exceeding 2/5/8; fewer is valid only when `synthesis.md` contains fewer distinct visual stories. For slides, build the deck. Read and follow `references/figure-generation-contract.md` before producing any pixels. Build visuals only from the claim and pattern entries in `synthesis.md`, never from a fresh reading of the papers. Probe the current agent's media capabilities first: when a capable built-in image generator is exposed, it is the default visual author for every non-quantitative figure, including comparison, evidence-map, timeline, popsci, bullets, and ELI5 work. Its first call renders the complete finished composition with every exact in-figure string directly in the pixels. Exact quantitative plots remain deterministic. Choose and record an explicit `generated`, `hybrid`, or `deterministic` route; hybrid is a last-resort repair after direct-text generation plus a targeted edit or alternate candidate fails, and `scripts/compose_hybrid_figure.py` then adds only the unresolved exact labels, arrows, legend, or values without resizing the base.

Every new figure spec uses `quality_contract_version: 1`, the review's explicit `review_style`, a numeric `target_aspect_ratio`, a `visual_anchor`, and an explicit `render_route`; include directed `relationships`, local `abbreviations`, and any shape-specific `geometry_invariants`. Build and save the mega-rich structured prompt with `scripts/build_figure_prompt.py`. Inspect the first complete direct-text candidate at original and final size; ship it when every scientific, copy, geometry, and visual-quality gate passes. Make one targeted ImageGen edit when a defect is local. Generate a genuinely different second candidate only when composition, style, science, or several text elements fail broadly, and record a comparison only when multiple candidates exist. Do not select a generic, cheap-looking flowchart merely because its text is easy to OCR. Hybrid is permitted only after the direct-text route and targeted repair or alternate candidate demonstrably fail, with `direct_text_attempted: true` and a concrete `fallback_reason` in `figure.provenance.json`. A non-quantitative deterministic fallback requires two generated candidates, a targeted edit, explicit hybrid consideration, and a concrete recorded failure reason.

Every deterministic text item in a hybrid figure requires its own explicit opaque background or a preceding opaque rectangle that fully covers the text envelope. Never place unmasked overlay text onto generated pixels, even when the region appears blank; keep a correct generated label, or erase the complete old label region before replacement. Record `duplicate_text` and `unlisted_text` as explicit lists in the visual inspection; either list being non-empty is a hard failure.

Never resize artwork or lettering with independent width and height scales. Circles remain circles, squares remain squares, and font proportions remain natural; fit copy by wrapping, moving, or editing it. Inspect the selected pixels and save the five visual-quality judgments plus `geometry_distortions` in `figure.inspection.json`, then run `python3 scripts/qa_figure.py --spec figure.json --image figure.png --inspection figure.inspection.json --provenance figure.provenance.json`. Blank or near-blank pixels, weak composition, weak hierarchy, generic subject matter, style mismatch, cheap finish, reported distortion, wrong raster aspect, incorrect copy/topology, or route/provenance shortcuts are hard failures. Give every passing figure a stable ID, introduce it before the artwork, and end its style-matched caption with 2–5 verified citations.
...
- `scripts/compose_hybrid_figure.py` — adds exact natural-width typography and vector-like annotations to generated artwork without resizing or distorting the base raster.
- `references/figure-generation-contract.md` and `references/figure-writing-style-overlays.json` — generator-first iteration, provenance, hybrid, visual-quality, non-distortion, and scientific/popsci/bullets/ELI5 art-direction contracts.
