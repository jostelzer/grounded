...
`Image`, `mindmap`, and `deck` are explicit-only media/delivery modes, not styles. Image combines with any size and style, with a size-scaled figure budget (small 1, medium up to 3, large up to 5 — caps, not quotas); mindmap uses the small tier and the selected writing style (scientific when none is named). Deck combines with any size and style and is itself the deliverable — a verified 16:9 PDF of standalone slides, with a 1–3 sentence plain answer in chat and the written synthesis kept as an internal working draft. Image and mindmap follow `media-modes.md`; deck follows `deck-guide.md`.
