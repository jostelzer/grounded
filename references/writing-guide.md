...
- **Agents act** (every style). The house sentence has a concrete subject doing something: the trial, the oil, the drug, the people, the number. "The defensible inference is possible modest benefit" has a concept as its subject and "is" as its verb; "The trials point to a modest benefit at best" has an agent and a verb. Sentences shaped "The [abstraction] is/remains/offers…" — the defensible inference, the appropriate conclusion, the practical boundary, the relevant question — are a rare deliberate move, never the default. A paragraph in which no concrete noun does anything gets rewritten.
- **Ration the antithesis** (every style). The verdict shape "X, not Y" ("modest benefit, not certainty"; "an average, not a forecast") is the strongest closing move available and the fastest to wear out. At most one per section, never two in a row, and never as the landing of consecutive sections. Land sections in varied shapes instead: on a number, on a consequence, on the answer to the section's question, on a plain declarative.
- **No coinages** (every style). If a compact phrase appears in neither the sources nor a dictionary — "biologically underspecified", "exposure separation", "the second necessary coordinate", "a mechanistic bridge" — unpack it into existing words ("the category is too mixed to test as one thing"). Invented abstractions read as precision but cost the reader a decoding step, and they are the strongest tell of machine prose.
