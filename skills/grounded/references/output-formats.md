...
Skip for inline chat. Journal PDFs require distinct, synthesis-grounded figures: use the generated target and cap for the selected size in `budgets.md`. Choose whole-answer, mechanism, study-design, quantitative, comparison or uncertainty views by their contribution to the explanation, in the order the article needs them. An overview is optional. Fewer figures is valid when the evidence earns fewer visual stories; do not fill slots with decoration. A popsci or ELI5 review may include a sectional cutaway under the ceiling when it removes a genuine imagination step, shows hidden structure faithfully and stays clear at 390 px (the `cutaway` archetype and its callout contract apply).
